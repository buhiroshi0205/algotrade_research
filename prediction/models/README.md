Each model is specified in its own file, and must implement `get_model(*args, **kwargs)` and `get_loss_fn()` functions. They they should be properly imported and referenced in `__init__.py`.
