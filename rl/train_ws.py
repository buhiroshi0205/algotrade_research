...
from with_state_env import StateEnv
...
def worker(hparams, seed, pipe):
    train_env = StateEnv(hparams['stocks'],hparams['train_split'],tc=hparams['tc'])
    eval_env = StateEnv(hparams['stocks'],hparams['val_split'],tc=hparams['tc'])
...
def run_once(experiment):
    ...
    run(params, n_trials=20, name='with_state' , experiment=experiment)
